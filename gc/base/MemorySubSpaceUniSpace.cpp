...
MM_MemorySubSpaceUniSpace::checkForRatioExpand(MM_EnvironmentBase *env, uintptr_t bytesRequired)
{
	...
	if (gcPercentage < _extensions->heapExpansionGCTimeThreshold._valueSpecified) {
...
MM_MemorySubSpaceUniSpace::checkForRatioContract(MM_EnvironmentBase *env) 
{
	 ...
	 */ 	
	if (gcPercentage > 0 && gcPercentage < _extensions->heapContractionGCTimeThreshold._valueSpecified) {
...
MM_MemorySubSpaceUniSpace::getHeapFreeMaximumHeuristicMultiplier(MM_EnvironmentBase *env)
{
	...
	uintptr_t expectedGcPercentage = (_extensions->heapContractionGCTimeThreshold._valueSpecified + _extensions->heapExpansionGCTimeThreshold._valueSpecified) / 2;
