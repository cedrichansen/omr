...
MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectResize(MM_EnvironmentBase *env)
{
	...
	if(extensions->dynamicNewSpaceSizing) {
		...
		if (doDynamicNewSpaceSizing) {
			double expectedTimeRatio = (extensions->dnssExpectedTimeRatioMaximum._valueSpecified + extensions->dnssExpectedTimeRatioMinimum._valueSpecified) / 2;
			...
			if(timeRatio > _averageScavengeTimeRatio) {
				if(timeRatio > expectedTimeRatio) {
					if(timeRatio > extensions->dnssExpectedTimeRatioMaximum._valueSpecified) {
			...
			if((_averageScavengeTimeRatio > extensions->dnssExpectedTimeRatioMaximum._valueSpecified)
			...
			if(_averageScavengeTimeRatio < extensions->dnssExpectedTimeRatioMinimum._valueSpecified
				...
				desiredContractionFactor = OMR_MIN(extensions->dnssExpectedTimeRatioMinimum._valueSpecified  * 2, expectedTimeRatio);
